...
readInput('inputs/00-example.txt').then((result) => {
  processData(result)
})

const createOutput = (demons) => {
  let output = "";
  demons.forEach(demon => { output += `${demon.index}\n` });
...
const processData = (data) => {
  ...
  const [staminaAmount, maxStamina, availableTurns, demonsAvailable] = parseLine(lines[0]);
  ...
  writeOutput('outputs/00-example.txt', createOutput(demons))
}
